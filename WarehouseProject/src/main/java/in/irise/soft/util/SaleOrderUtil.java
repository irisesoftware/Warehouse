...
public class SaleOrderUtil {
	...
	public void generatePieChart(String location, List<Object[]> list) {
		...
		JFreeChart chart = ChartFactory.createPieChart3D("Sale Order Mode", dataset);
		
		//// c. Convert JFreeChart object into one Image using ChartUtil class
		try {
			ChartUtils.saveChartAsJPEG(new File(location + "/saleorderA.jpg"), chart, 400, 300);		
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public void generateBarChart(String location,List<Object[]> list) {
		...
		// b. Convert DataSet data into JFreeChart object using ChartFactory class
		JFreeChart chart=ChartFactory.createBarChart("Sale Order Mode", "QUALITY", "COUNT", dataset);
		
	//// c. Convert JFreeChart object into one Image using ChartUtil class
			try {
				ChartUtils.saveChartAsJPEG(new File(location + "/saleorderB.jpg"), chart, 500, 340);		
			} catch (IOException e) {
				e.printStackTrace();
			}
	}
